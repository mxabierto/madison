...
class SponsorCommand extends Command
{
    ...
    public function fire()
    {
        ...
        foreach ($docs as $doc) {
            ...
            switch ($sponsors[$selected]['sponsor_type']) {
                case 'individual':
                    $doc->userSponsor()->sync([$sponsors[$selected]['id']]);
                    $this->info("Assigned Document to Independent Sponsor");
                    break;
                case 'group':
                    $doc->groupSponsor()->sync([$sponsors[$selected]['id']]);
    ...
    {
        return [
            ['docId', InputArgument::OPTIONAL, 'An optional document to change the sponsor of.'],
        ];
    }
